...
class DifixPipeline(
    ...
    def prepare_latents(self, image, batch_size, num_images_per_prompt, dtype, device, generator=None):
        ...
        # image = image.to(device=device, dtype=dtype) # Handled externally in our simple version
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if ref_image is not None:
            ref_image = self.image_processor.preprocess(ref_image).to(device=device, dtype=prompt_embeds.dtype)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # NVIDIA's secret sauce for 3D consistency
                if hasattr(self.vae.decoder, "incoming_skip_acts") and hasattr(self.vae.encoder, "current_down_blocks"):
